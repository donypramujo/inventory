...
class BuilderTableCreateDojoInventoryStocks extends Migration {
	public function up() {
		Schema::create ( 'dojo_inventory_stocks', function ($table) {
			...
			$table->foreign ( 'product_id' )->references ( 'id' )->on ( 'dojo_inventory_products' )->onUpdate ( 'restrict' )->onDelete ( 'restrict' );
			
			$table->integer ( 'location_id' )->unsigned ();
			$table->foreign ( 'location_id' )->references ( 'id' )->on ( 'dojo_inventory_locations' )->onUpdate ( 'restrict' )->onDelete ( 'restrict' );
			
			$table->integer ( 'created_by' )->unsigned ()->nullable ();
			$table->foreign ( 'created_by' )->references ( 'id' )->on ( 'backend_users' )->onUpdate ( 'restrict' )->onDelete ( 'restrict' );
			
			$table->integer ( 'updated_by' )->unsigned ()->nullable ();
			$table->foreign ( 'updated_by' )->references ( 'id' )->on ( 'backend_users' )->onUpdate ( 'restrict' )->onDelete ( 'restrict' );
		...
		DB::statement ( 'CREATE VIEW dojo_inventory_summary_of_stocks 
							...
							AS status_count,dojo_inventory_stocks.product_id 
							AS product_id,dojo_inventory_stocks.location_id 
							AS location_id from dojo_inventory_stocks 
							GROUP BY dojo_inventory_stocks.product_id,dojo_inventory_stocks.location_id,dojo_inventory_stocks.status' );
	}
